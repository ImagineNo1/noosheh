...
export default function Products() {
  ...
  return <div className="admin-page">
    ...
    <Dialog open={dialogOpen} title={editingProduct ? 'ویرایش محصول' : 'افزودن محصول جدید'} onClose={closeDialog} wide><div className="admin-form"><div className="store-tab-list">{[['basic', 'اطلاعات پایه'], ['images', 'تصاویر'], ['variants', 'رنگ، سایز و کاپ'], ['content', 'توضیحات'], ['relations', 'محصولات مرتبط'], ['seo', 'SEO']].map(([key, label]) => <button key={key} type="button" className={editorTab === key ? 'active' : ''} onClick={() => setEditorTab(key)}>{label}</button>)}</div>{saveError && <div className="admin-alert destructive">{saveError}</div>}
      ...
      {editorTab === 'relations' && <div className="admin-manager-stack"><div className="admin-soft-box">اگر محصولی انتخاب نکنید، صفحه محصول به‌صورت خودکار پیشنهادها را بر اساس شباهت‌ها نمایش می‌دهد.</div><AdminRelationPicker label="Complete the Look" products={products} excludeId={editingProduct?.id} contextProduct={{ ...form, id: editingProduct?.id || '' } as Product} relationType="complete" selectedIds={form.complete_the_look_ids || []} onChange={(ids) => updateField('complete_the_look_ids', ids)} /><AdminRelationPicker label="محصولات مشابه" products={products} excludeId={editingProduct?.id} contextProduct={{ ...form, id: editingProduct?.id || '' } as Product} relationType="similar" selectedIds={form.similar_product_ids || []} onChange={(ids) => updateField('similar_product_ids', ids)} /></div>}
