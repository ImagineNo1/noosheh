...
export function productIdentifierMatches(product: Partial<Pick<Product, 'id' | 'code' | 'slug' | 'title' | 'name'>>, id: string) {
  const normalizeIdentifier = (value: unknown) => {
    const raw = stringifyProductValue(value).trim();
    if (!raw) return '';
    try {
  ...
  const target = normalizeIdentifier(id);
  const titleSlug = slugifyProductValue(stringifyProductValue(product.title ?? product.name));
  return Boolean(target) && [product.slug, product.code, product.id, titleSlug].some((value) => normalizeIdentifier(value) === target);
}
...
export function productCanonicalUrl(canonicalUrl: unknown, product: Partial<Pick<Product, 'id' | 'code' | 'slug' | 'title' | 'name'>>, siteUrl?: string) {
  ...
  if (!raw) return '';
  try {
    const origin = (siteUrl || 'https://noosheh.com').replace(/\/$/, '');
