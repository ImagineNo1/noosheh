...
export default function SeoTab({ entity, entityType, entityId }: { entity: any; entityType: string; entityId: string }) {
  ...
  const canonical = form.canonical_url || `https://example.com/${entityType}/${entityId}`;
  ...
  return <section className='space-y-4 rounded-2xl border bg-card p-4'>
    ...
    <div className='grid gap-3 sm:grid-cols-2'>
      ...
      <Field label='Canonical URL' hint='آدرس canonical کامل و absolute'>
    ...
    <div className='grid gap-3 sm:grid-cols-2'>
      <Field label='OG Title'><input className='w-full rounded-lg border p-2' value={form.og_title || ''} onChange={(e) => setForm((f: any) => ({ ...f, og_title: e.target.value }))} /></Field>
      <Field label='OG Image URL'><input className='w-full rounded-lg border p-2' dir='ltr' value={form.og_image || ''} onChange={(e) => setForm((f: any) => ({ ...f, og_image: e.target.value }))} /></Field>
      <Field label='OG Description'><textarea className='w-full rounded-lg border p-2 text-sm min-h-20' value={form.og_description || ''} onChange={(e) => setForm((f: any) => ({ ...f, og_description: e.target.value }))} /></Field>
      <Field label='Twitter Title'><input className='w-full rounded-lg border p-2' value={form.twitter_title || ''} onChange={(e) => setForm((f: any) => ({ ...f, twitter_title: e.target.value }))} /></Field>
      <Field label='Twitter Image URL'><input className='w-full rounded-lg border p-2' dir='ltr' value={form.twitter_image || ''} onChange={(e) => setForm((f: any) => ({ ...f, twitter_image: e.target.value }))} /></Field>
      <Field label='Twitter Description'><textarea className='w-full rounded-lg border p-2 text-sm min-h-20' value={form.twitter_description || ''} onChange={(e) => setForm((f: any) => ({ ...f, twitter_description: e.target.value }))} /></Field>
    </div>
    ...
    <Field label='Custom JSON-LD' hint='در صورت نیاز Schema سفارشی وارد کنید'>
