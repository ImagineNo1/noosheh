  ...
  return (
    <Link href={`/product/${product.id}`} className="group block">
      <div className="overflow-hidden rounded-lg border border-border bg-card transition-shadow hover:shadow-md">
        ...
        <div className="space-y-1.5 p-3"><h3 className="line-clamp-2 text-sm font-medium">{product.title}</h3><div className="flex items-center gap-2"><span className="text-sm font-bold text-primary">{formatPrice(price)} تومان</span>{hasDiscount && <span className="text-xs text-muted-foreground line-through">{formatPrice(product.price)}</span>}</div>{product.avg_rating ? <div className="text-xs text-muted-foreground">★ {product.avg_rating.toFixed(1)} ({product.review_count || 0})</div> : null}{colors.length > 0 && <div className="flex gap-1 pt-1">{colors.slice(0, 5).map((color) => <span key={color.value} className="h-4 w-4 rounded-full border border-border" style={{ backgroundColor: color.hex || color.value }} />)}{colors.length > 5 && <span className="self-center text-[10px] text-muted-foreground">+{colors.length - 5}</span>}</div>}</div>
      </div>
    </Link>
  );
}
